...
class LogInFrame(Frame):
    def __init__(self, master):
        ...
        show_password_checkbox = Checkbutton(self, text='Показать пароль', font='Arial 9',
                                             variable=self.show_password_var, command=self.password_visibility)
...
class SignInFrame(Frame):
    ...
    def check_entry_fields_sigin(self):
        return bool(
            self.username_entry.get() and self.email_entry.get() and self.password1_entry.get() and self.password2_entry.get())
