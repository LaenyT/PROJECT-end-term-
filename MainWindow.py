...
class DatabaseManager:
    ...
    def save_file(self, filename, encoded_content, email, favorite):
        cursor = self.conn_files.cursor()
        cursor.execute('INSERT INTO files (filename, content, user_email, favorite) VALUES (?, ?, ?, ?)',
                       (filename, encoded_content, email, favorite))
        self.conn_files.commit()
...
class MainCloudWindow:
    ...
    def create_tabs(self):
        ...
        personal_tab = PersonalTab(self.tab_control, db_manager=self.db_manager, username=self.username,
                                   email=self.email)
        all_files_tab = AllFilesTab(self.tab_control, db_manager=self.db_manager, username=self.username,
                                    email=self.email)
        favorites_tab = FavoritesTab(self.tab_control, db_manager=self.db_manager, username=self.username,
                                     email=self.email)
...
class AllFilesTab(ttk.Frame):
    ...
    def add_favorite(self):
        if hasattr(self, 'selected_file'):
            ...
            cursor.execute('UPDATE files SET favorite=? WHERE filename=? AND user_email=?',
                           (favorite, self.selected_file, self.email))
    ...
    def download_file(self):
        if hasattr(self, 'selected_file'):
            cursor = self.db_manager.conn_files.cursor()
            cursor.execute('SELECT filename, content FROM files WHERE filename=? AND user_email=?',
                           (self.selected_file, self.email))
    ...
    def move_basket(self):
        if hasattr(self, 'selected_file'):
            result = messagebox.askquestion("Удаление файла",
                                            f"Вы уверены, что хотите удалить файл {self.selected_file}?")
            ...
            if result == "yes":
                ...
                cursor.execute('INSERT INTO basket_files (filename, content, user_email) VALUES (?, ?, ?)',
                               (filename, encoded_content, self.email))
...
class FavoritesTab(ttk.Frame):
    ...
    def delete_favorites(self):
        if hasattr(self, 'selected_file'):
            result = messagebox.askquestion("Удаление из Избранных",
                                            f"Вы уверены, что хотите удалить файл {self.selected_file}?")
...
class TrashTab(ttk.Frame):
    ...
    def restore_file(self):
        if hasattr(self, 'selected_file'):
            result = messagebox.askquestion("Восстановлени файла",
                                            f"Вы уверены, что хотите восстановить файл {self.selected_file}?")

            if result == "yes":
                cursor = self.db_manager.conn_basket_files.cursor()
                cursor.execute('SELECT filename, content FROM basket_files WHERE filename=? AND user_email=?',
                               (self.selected_file, self.email))
                file_info = cursor.fetchone()

                filename, encoded_content = file_info
                cursor = self.db_manager.conn_files.cursor()
                cursor.execute('INSERT INTO files (filename, content, user_email) VALUES (?, ?, ?)',
                               (filename, encoded_content, self.email))
                self.db_manager.conn_files.commit()

                cursor = self.db_manager.conn_basket_files.cursor()
                cursor.execute('DELETE FROM basket_files WHERE filename=? AND user_email=?',
                               (self.selected_file, self.email))
                self.db_manager.conn_basket_files.commit()
